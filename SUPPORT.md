# support

## questions

read the [README](README.md), [pack reference](docs/positioning-pack.md), and
[MCP guide](docs/mcp.md) first.

after a public remote exists, use GitHub Discussions for setup and design questions. use
issues only for reproducible bugs and bounded feature requests. the initial project does
not promise paid support, response times, or private implementation consulting.

## useful bug report

include the package version, Node.js version, operating system, command or MCP host,
smallest fictional pack and draft that reproduce the issue, expected decision, actual
decision, and whether semantic review was configured.

remove company-confidential positioning, proof, and draft content before posting. use
the private process in [SECURITY.md](SECURITY.md) for vulnerabilities.
